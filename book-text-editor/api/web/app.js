  ...
  class AudiobookApp {
    ...
    async openJob(jobID, announce) {
      ...
      try {
        ...
        if (TERMINAL_JOB_STATUSES.has(job.status)) {
          await this.loadWarnings(normalizedJobID);
          return;
        }
    ...
    async loadWarnings(jobID) {
      try {
        ...
        const fragments = Array.isArray(response.fragments)
          ? response.fragments
          : [];
        this.renderWarnings(fragments);
        if (fragments.length > 0) {
          this.loadRewriteModels();
        }
      } catch (error) {
        this.notifyError(error, "Не удалось загрузить спорные фрагменты");
      }
    }
      ...
      await this.withBusy(
        ...
        async () => {
          ...
          const checkbox = this.document.getElementById(
            `warning-select-${this.safeDOMID(fragment.id)}`,
          );
          if (checkbox) {
          ...
          if (this.currentJobID) {
            await this.loadWarnings(this.currentJobID);
          }
          this.notify("Правка сохранена. Фрагмент отмечен для повтора.", "success");
        },
      );
    }
    ...
    async retryWarnings(fragmentIDs) {
      ...
      if (
        ...
        this.nodes.warningsList.childElementCount === 0
      ) {
    ...
    renderChapterDownloads(job, chapters) {
      ...
      if (normalized.length === 0) {
        this.nodes.chapterDownloadsStatus.textContent =
          "В готовой задаче нет собранных глав FLAC.";
        return;
      }

      this.nodes.chapterDownloadsStatus.textContent =
        `Готовых глав FLAC: ${normalized.length}.`;
      for (const chapter of normalized) {
        const flacFilename = this.chapterFLACFilename(chapter);
        const link = this.createNode("a", "chapter-download-link");
        link.href =
          `/v1/job/${encodeURIComponent(job.id)}/chapters/` +
          `${encodeURIComponent(String(chapter.chapter_number))}/audio.zip`;
        link.download =
          "ready-chapter-flac-" +
          `${String(chapter.chapter_number).padStart(4, "0")}.zip`;
        link.setAttribute(
          "aria-label",
          `Скачать готовую главу FLAC ${chapter.chapter_number}: ` +
            `${chapter.title || "Без названия"}. ` +
            `ZIP содержит ${flacFilename}`,
        );
        link.title = `ZIP содержит готовую главу ${flacFilename}`;
        ...
        const detail = this.createNode(
          "small",
          "",
          `${flacFilename} · один FLAC в ZIP`,
        );
        copy.append(title, detail);
        const arrow = this.createNode("span", "chapter-download-arrow", "↓");
        arrow.setAttribute("aria-hidden", "true");
        link.append(number, copy, arrow);
        this.nodes.chapterDownloadsList.append(link);
      }
    }
    ...
    renderWarnings(fragments) {
      ...
      this.nodes.warningsSummary.textContent =
        `Найдено фрагментов для проверки: ${fragments.length}.`;

      for (const fragment of fragments) {
        const card = this.createWarningCard(fragment);
